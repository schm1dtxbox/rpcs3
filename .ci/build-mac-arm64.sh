...
brew_arm64_install_packages() {
    for pkg in "$@"; do
        ...
        bottle_path="$("$BREW_ARM64_PATH/bin/brew" --cache --bottle-tag=arm64_sequoia "$pkg")"
        if [ ! -f "$bottle_path" ]; then
            if ! "$BREW_ARM64_PATH/bin/brew" fetch --force --verbose --debug --bottle-tag=arm64_sequoia "$pkg"; then
...
sudo rm -rf /opt/homebrew/Cellar/curl /opt/homebrew/opt/curl
/opt/homebrew/bin/brew install -f --overwrite curl
/opt/homebrew/bin/brew uninstall -f --ignore-dependencies ffmpeg
/opt/homebrew/bin/brew install -f ffmpeg@5 || true
/opt/homebrew/bin/brew install -f --overwrite python || true
/opt/homebrew/bin/brew link --overwrite python || true
/opt/homebrew/bin/brew install -f --overwrite nasm ninja p7zip ccache pipenv #create-dmg
/opt/homebrew/bin/brew link -f curl || true
/opt/homebrew/bin/brew install llvm@$LLVM_COMPILER_VER glew cmake sdl2 vulkan-headers coreutils
/opt/homebrew/bin/brew link -f llvm@$LLVM_COMPILER_VER ffmpeg@5 || true
...
BREW_X64_PATH="$("/opt/homebrew/bin/brew" --prefix)"
